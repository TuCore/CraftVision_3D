      ...
      <div className="glass-card rounded-2xl p-4 flex flex-col md:flex-row gap-4 items-center shadow-soft">
        ...
        <div className="relative w-full md:w-56 shrink-0">
          ...
          <Select value={statusFilter} onValueChange={setStatusFilter}>
            <SelectTrigger className="w-full bg-white border border-border rounded-xl pl-9 pr-4 h-[42px] text-sm font-medium hover:border-[color:var(--coral)]/50 focus:ring-0 focus:ring-offset-0 focus:border-[color:var(--coral)]/50 transition-colors shadow-none">
              <SelectValue placeholder="Tất cả trạng thái" />
            </SelectTrigger>
            <SelectContent className="rounded-2xl border border-border bg-white shadow-xl p-1.5 animate-in fade-in-80 zoom-in-95">
              ...
              {isLoading ? (
                ...
                paginatedTags.map((tag) => (
                  <tr key={tag.id} className="hover:bg-muted/30 transition-colors">
                    <td className="px-6 py-4 font-semibold text-foreground whitespace-nowrap">{tag.tagCode}</td>
                    <td className="px-6 py-4 whitespace-nowrap">{getStatusBadge(tag.status)}</td>
                    <td className="px-6 py-4 text-center whitespace-nowrap">
                      <span className="bg-[color:var(--coral)]/10 text-[color:var(--coral)] font-bold px-2.5 py-1 rounded-lg">{tag.scanCount}</span>
                    </td>
                    <td className="px-6 py-4 text-muted-foreground whitespace-nowrap">{formatDate(tag.lastScanAt)}</td>
                    <td className="px-6 py-4 text-right space-x-2 flex justify-end whitespace-nowrap">
                      {tag.status === "Disabled" && (
                        <button
                          onClick={() => handleUpdateStatus(tag.tagCode, "Available")}
                          className="px-3 py-1.5 bg-green-100 text-green-700 hover:bg-green-200 rounded-lg font-semibold transition-colors flex items-center gap-1.5"
                          title="Enable NFC"
                        >
                          <Power className="w-4 h-4" /> Bật
                        </button>
                      )}
                      {tag.linkedUrl && (
                        <button
                          onClick={() => window.open(tag.linkedUrl || '', '_blank')}
          ...
          {selectedTag && (
            <div className="space-y-6">
              <div className="grid grid-cols-2 gap-y-4 text-sm">
                ...
                <div className="font-medium text-foreground text-right">{formatDate(selectedTag?.createdAt || null)}</div>

                <div className="text-muted-foreground font-medium">Last Scan</div>
                <div className="font-medium text-foreground text-right">{formatDate(selectedTag?.lastScanAt || null)}</div>
              ...
              <div className="grid grid-cols-2 gap-3 pt-4 border-t border-border">
                ...
                <button
                  onClick={() => selectedTag && handleResetScan(selectedTag.tagCode)}
                ...
                {selectedTag?.status === "Active" ? (
                  <button
                    onClick={() => selectedTag && handleUpdateStatus(selectedTag.tagCode, "Disabled")}
                  ...
                  <button
                    onClick={() => selectedTag && handleUpdateStatus(selectedTag.tagCode, "Active")}
