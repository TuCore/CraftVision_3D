...
BEGIN
    ...
    FOR rec IN SELECT id, name FROM products LOOP
        ...
        ELSIF rec.name ILIKE '%móc khóa%' THEN img_url := '/image/mockhoa2.jpg';
        ELSIF rec.name ILIKE '%hoa%' THEN img_url := '/image/hoa.png';
        ELSIF rec.name ILIKE '%vòng cổ%' THEN img_url := '/image/vongco.jpg';
        ELSIF rec.name ILIKE '%vòng tay%' THEN img_url := '/image/vongtay.jpg';
